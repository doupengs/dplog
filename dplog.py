...
class Logger(object):
    ...
    @staticmethod
    def addColor(obj, textColor=None, bgColor=None, style=None, color=True):
        '''
        ...
        :param color: bool
        :function: return obj with|without color
        '''
        if color == False:
            return obj
        if color == True:
            ...
            return '%s%s%s%s\033[0m' % (textColor, bgColor, style, obj)
    ...
    def _makeCLogger(logName, colorType):
        ...
        format = logging.Formatter(
            Logger.addColor(Logger.LOG_FORMAT, textColor, bgColor, style),
            Logger.TIME_FORMAT
        )
